...
export default function PurchaseReport() {
  ...
  const { data, isLoading } = useFetchData({
    method: "GET",
    path: `factory/purchase/monthly-purchase`,
    queryKey: "getPurchaseReportData",
    filterData: {
  ...
  const rowsWithSl = reportData.map((item: any, index: any) => ({
    sl: index + 1,
    ...item,
  }));
          ...
          config={{
            columns: [
              ...
              },
              {
                ...
                render: (row) => (
                  <span
                    className={`${row.sl === null ? "font-semibold" : ""} text-orange-600`}
                  >
